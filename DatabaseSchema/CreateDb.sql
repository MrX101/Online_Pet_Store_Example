...
create or replace function validate_permission_allowed_page_ids() returns trigger
    language plpgsql
as
$$
DECLARE current_id INTEGER;
BEGIN
    FOR i IN 1..array_length(NEW.allowed_page_ids, 1) LOOP
            current_id := NEW.allowed_page_ids[i];
            IF NOT EXISTS (SELECT 1 FROM pages WHERE id = current_id) THEN
                RAISE EXCEPTION 'Invalid page_id % in allowed_page_ids column', current_id;
            END IF;
        END LOOP;
    RETURN NEW;
END;
...
alter function validate_permission_allowed_page_ids() owner to postgres;

create trigger validate_array_column_trigger
    before insert or update
    on permission
    for each row
execute procedure validate_permission_allowed_page_ids();
